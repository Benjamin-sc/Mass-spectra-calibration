...
class GUI_spectra:
    def __init__(self, parent):
        ...
        self.subframe1b = LabelFrame(self.frame1,text = "Compute ion time of flight", bg="white", fg="black", font='15')
        self.subframe1b.place(relwidth=0.6,relheight=0.18, relx=0.3, rely=0.005)
        ...
        self.data_conv_mass        = None
        self.data_conv_volt        = None
        ...
        self.buttonComputeToF = Button(self.subframe1b)
        self.buttonComputeToF.configure(text="Compute",
                                        bg = "Steel Blue",
                                        fg = "White",
                                        activeforeground = "White",
                                        activebackground = "Black",
                                        command = self.compute_tof)
        self.Edit_mass = Entry(self.subframe1b, width= 8)
        self.Edit_mass.insert(END, 40)
        self.Edit_energy = Entry(self.subframe1b, width= 8)
        self.Edit_energy.insert(END, 1000)
        self.Edit_distance = Entry(self.subframe1b, width= 8)
        self.Edit_distance.insert(END, 1.5)
        self.Edit_time = Entry(self.subframe1b, width= 10)

        
        self.Label_mass= Label(self.subframe1b,text="mass(amu)",bg = "grey", fg="white")
        self.Label_energy= Label(self.subframe1b,text="E(eV)",bg = "grey", fg="white")
        self.Label_distance= Label(self.subframe1b,text="distance(m)",bg = "grey", fg="white")
        self.Label_time= Label(self.subframe1b,text="time (µs)",bg = "grey", fg="white")
        ...
        self.buttonwidth = Button(self.frame2)
        self.buttonwidth.configure(text="Save spectrum data",
                                        bg = "Steel Blue",
                                        fg = "White",
                                        activeforeground = "White",
                                        activebackground = "Black",
                                        command = self.Save_spectra)
        ...
        # Compute ToF (subframe1b)
        
        self.Edit_mass.grid(column = 1, row = 0,sticky = "EW")
        self.Edit_energy.grid(column = 1, row = 1,sticky = "EW")
        self.Edit_distance.grid(column = 1, row = 2,sticky = "EW")
        self.Edit_time.grid(column = 4, row = 1,sticky = "EW")
        
        self.Label_mass.grid(column = 0, row = 0,sticky = "EW")
        self.Label_energy.grid(column = 0, row = 1,sticky = "EW")
        self.Label_distance.grid(column = 0, row = 2,sticky = "EW")
        self.Label_time.grid(column = 3, row = 1,sticky = "EW")
        
        self.buttonComputeToF.grid(column = 2, row = 0, rowspan = 3, sticky = "NESW")
        ...
        self.buttonwidth.grid(column = 4, row = 2,sticky = "EW")
    ...
    def Calibrate(self):
        ...
        self.data_conv_mass = self.coeffs_fit_poly[0]*data[:,0]**2+self.coeffs_fit_poly[1]*data[:,0] +self.coeffs_fit_poly[2]
        self.data_conv_volt = data[:,1]
        

        fig = plt.figure(figsize=(6.5, 4.5), dpi=90)
        fig.add_subplot(111).plot(self.data_conv_mass,self.data_conv_volt,'r', linewidth = 0.5)
        plt.xlabel('mass (m/z)')
        plt.ylabel('volt (mV)')
    ...
    def Save_spectra(self):
        ...
        df = pd.DataFrame({'Mass': self.data_conv_mass,'volt': self.data_conv_volt})
        writer = pd.ExcelWriter('spectrum_calibrated.xlsx', engine='xlsxwriter')
        df.to_excel(writer, sheet_name='Sheet1', index=False)
        writer.save()
        
    def compute_tof(self):
        try:
            mass = float(self.Edit_mass.get())
            E = float(self.Edit_energy.get())
            d = float(self.Edit_distance.get())
        except: 
            messagebox.showinfo ("warning","be shure to enter a mass, energy and distance of flight")
        


        E_SI = E*1.6*10**(-19)
        mass_SI = mass*1.66*10**(-27)
        
        time = (mass_SI/(2*E_SI))**0.5*d
        
        time_µs = time*10**6
        
        self.Edit_time.insert(0, time_µs)
